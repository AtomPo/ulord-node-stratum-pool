			...
			function(result){
                var validResults = results.filter(function (r) {
                    if (r.error) {
                        ...
                        r.instance.index + ' with error ' + JSON.stringify(r.error));
                        return false;
                    }
                    return true;
                })
                ...
                if (validResults.length > 0) {
                    var maxHeight = 0;
                    var maxResponse = null;
                    validResults.forEach(function (r) {
                        if (r.response.height > maxHeight) {
                            maxResponse = r.response;
                            maxHeight = r.response.height;
                        }
                    })
                    var processedNewBlock = _this.jobManager.processTemplate(maxResponse);
                    callback(null, rmaxResponse, processedNewBlock);
                    callback = function () { };
                } else {
                    if (result.length > 0) {
